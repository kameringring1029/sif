...
class UseCV:
	...
	def analyze_eventSC(self, filename):
		...
		commands.getoutput("tesseract /var/www/up/pink_image.png /var/www/up/now -psm 7 whitelist")
	...
	#	trim_image = image[870:920, 350:500]
		...
		commands.getoutput("tesseract /var/www/up/gray_image.png /var/www/up/item -psm 7 whitelist")
	...
	def analyze_LP(self,filename):
		...
		commands.getoutput("tesseract /var/www/up/rank_image.png /var/www/up/rank -psm 7 whitelist")
		lp_image = image[height_orig*0.07:height_orig*0.099, width_orig*0.73:width_orig*0.81]
		cv2.imwrite(os.path.join('/var/www/up/', 'lp_image.png'), lp_image)
	    # グレースケールに変換
		lp_image = cv2.cvtColor(lp_image, cv2.COLOR_BGR2GRAY)
		cv2.imwrite(os.path.join('/var/www/up/', 'lp_image.png'), lp_image)
		commands.getoutput("tesseract /var/www/up/lp_image.png /var/www/up/lp -psm 7 whitelist")
	
		exp_image = image[height_orig*0.07:height_orig*0.099, width_orig*0.55:width_orig*0.70]
		cv2.imwrite(os.path.join('/var/www/up/', 'exp_image.png'), exp_image)
		commands.getoutput("tesseract /var/www/up/exp_image.png /var/www/up/exp -psm 7 whitelist")
