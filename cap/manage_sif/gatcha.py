		...
		if (status.text.find('@sif_notify_bot') > -1) and (status.text.find('RT') < 0) :
			...
			for num in range(0,11):
				...
				while member_id > 27 and member_id < 52:
					...
					series = 0
					if rarity > 1:
						print str(status.text.rsplit(' ',1)[1].encode('utf-8'))
						series = str(status.text.rsplit(' ',1)[1].encode('utf-8'))
						if not('編' in series):
							series = 0
				...
				if (member_name != 'アルパカ' and member_rare == 'R') or (member_name == 'アルパカ' and member_rare == 'SR'):
					...
					member_info = member_rare + member_name + "(" + member_series + ")"
				...
				filename = '/home/pi/sif/cap/manage_sif/icon'+str(num)+'.png'
