...
class UseSQL:
	...
	## ユーザ情報更新関数 ##
	########################
	def update_statusTable(self, time_status, username):
	...
	def get_member(self, member_id):
		...
		try:
 			##select
			sql = "select name, series, rarity, S_0, P_0, C_0, type, image_url from carddata where id="+str(member_id) 
	#		print "[SQL]"+sql
			cursor.execute(sql)
...
#				spc = "クール"
	...
	##################
	def rand_member(self, rarity, names):
		print "rand_member"
		connector = MySQLdb.connect(host="localhost", db="sif_event", user="web", passwd="password", charset="utf8")
		cursor = connector.cursor()
...
#				if rarity == 4:
	...
	def get_member_info(self, member_id):
		...
		try:
 			##select
			sql = "select name, series, rarity, S_1, P_1, C_1, type, unit_skill, fullimg_1, imple_info from carddata where id="+str(member_id) 
	#		print "[SQL]"+sql
			cursor.execute(sql)
			result = cursor.fetchall()[0]
			name = result[0].encode('utf-8')
			...
			imple_info = result[9].encode('utf-8')
			...
			fullimg_0 = result[8].encode('utf-8')
